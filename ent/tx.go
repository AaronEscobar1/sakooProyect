...
func (tx *Tx) init() {
	...
	tx.PaymentCommitment = NewPaymentCommitmentClient(tx.config)
	tx.PaymentNotification = NewPaymentNotificationClient(tx.config)
	tx.ResponseCode = NewResponseCodeClient(tx.config)
	tx.ThirdPartyAccount = NewThirdPartyAccountClient(tx.config)
